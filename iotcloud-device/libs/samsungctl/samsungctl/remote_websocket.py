...
class RemoteWebsocket(websocket_base.WebSocketBase):
    ...
    def has_ssl(self):
        try:
            response = requests.get(
                " http://{0}:8001/api/v2/".format(self.config.host), timeout=3
            )
            return json.loads(response.content.decode("utf-8"))["device"][
                "TokenAuthSupport"
            ]
        ...
        with self.receive_lock:
            ...
            def unauthorized_callback(_):
                ...
                self.unregister_receive_callback(
                    auth_callback, "event", "ms.channel.connect"
                )

                if self.config.port == 8001:
                    logger.debug("Websocket connection failed. Trying ssl connection")
                    self.config.port = 8002
                    self.open()
                else:
                    self.close()
            ...
            def auth_callback(data):
                ...
                self.unregister_receive_callback(
                    unauthorized_callback, "event", "ms.channel.unauthorized"
                )
            ...
            self.register_receive_callback(auth_callback, "event", "ms.channel.connect")
    ...
    def control(self, key, cmd="Click"):
        ...
        with self.receive_lock:
            params = dict(
                Cmd=cmd, DataOfCmd=key, Option="false", TypeOfRemote="SendRemoteKey"
            )
    ...
    def applications(self):
        ...
        self.register_receive_callback(
            installed_app_get, "event", "ed.installedApp.get"
        )

        for event in ["ed.edenApp.get", "ed.installedApp.get"]:
            params = dict(data="", event=event, to="host")
        ...
        self.unregister_receive_callback(eden_app_get, "event", "ed.edenApp.get")

        self.unregister_receive_callback(installed_app_get, "data", None)
                ...
                if event == "d2d_service_message":
                    ...
                    if "event" in data:
                        for callback, key, _ in self._registered_callbacks[:]:
                            if key == data["event"]:
                                callback(data)
                                self._registered_callbacks.remove([callback, key, None])
                                break
    ...
    def artmode(self):
        ...
        params = dict(
            ...
            data=json.dumps(dict(request="get_artmode_status", id=self.config.id)),
        ...
        self.register_receive_callback(artmode_callback, "artmode_status", None)

        self.send("ms.channel.emit", **params)

        event.wait(2.0)

        self.unregister_receive_callback(artmode_callback, "artmode_status", None)
    ...
    def artmode(self, value):
        ...
        params = dict(
            clientIp=socket.gethostbyname(socket.gethostname()),
            data=json.dumps(
                dict(request="set_artmode_status", value=value, id=self.config.id)
            ),
            deviceName=self._serialize_string(self.config.name),
            event="art_app_request",
            to="host",
    ...
    def start_voice_recognition(self):
        ...
        with self.receive_lock:
            ...
            self.register_receive_callback(
                voice_callback, "event", "ms.voiceApp.standby"
            )
    ...
    def stop_voice_recognition(self):
        ...
        with self.receive_lock:
            ...
            self.register_receive_callback(voice_callback, "event", "ms.voiceApp.hide")
            ...
            event.wait(2.0)
            self.unregister_receive_callback(
                voice_callback, "event", "ms.voiceApp.hide"
            )
            if not event.isSet():
...
class Mouse(object):
    ...
    def _send(self, cmd, **kwargs):
        ...
        if not self.is_running:
            params = {"Cmd": cmd, "TypeOfRemote": "ProcessMouseDevice"}
            params.update(kwargs)

            payload = json.dumps({"method": "ms.remote.control", "params": params})
    ...
    @LogIt
    def move(self, x, y):
        position = dict(x=x, y=y, Time=str(time.time()))
        ...
        if not self.is_running:
            ...
            with self._remote.receive_lock:
                ...
                self._remote.register_receive_callback(
                    ime_start, "event", "ms.remote.imeStart"
                )

                self._remote.register_receive_callback(
                    ime_update, "event", "ms.remote.imeUpdate"
                )

                self._remote.register_receive_callback(
                    touch_enable, "event", "ms.remote.touchEnable"
                )
                ...
                for payload in self._commands:
                    if isinstance(payload, (float, int)):
                        ...
                        logger.info("Sending mouse control command: " + str(payload))
